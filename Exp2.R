...
##------------------------First test (station/pipes)--------------------------##
    ...
    exp.time3(type = "Station", unit = "Temp", time.un = "mins")
        test1.station.temp <- as.matrix(est.obs)
    exp.time3(type = "Station", unit = "Humd", time.un = "mins")
        test1.station.humd <- as.matrix(est.obs)
    exp.time3(type = "Pipes", unit = "Temp", time.un = "mins")
        test1.pipes.temp <- as.matrix(est.obs)
    exp.time3(type = "Pipes", unit = "Humd", time.un = "mins")
...
stationdata[,2] <- as.POSIXct(rownames(stationdata))
stationdata[,3] <- 1:length(stationdata[,1])
ggplot(data=stationdata, aes(x=V3, y=stationdata)) + geom_area(aes(alpha=0.01, fill="red")) + 
  theme(text = element_text(size=20)) + ylab("Degrees Celsius") + xlab("Minutes") +
  coord_cartesian(ylim = c(18, 34)) + geom_area(aes(y=pipedata$pipedata, alpha=0.01, fill="blue")) +
  geom_line(aes(y=pipedata$pipedata), position="stack") + geom_line(aes(y=stationdata), position="stack") 
