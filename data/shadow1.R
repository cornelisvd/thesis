                       ...
                       cr.gif    = TRUE)               # Create an animation
{
    ...
    poly    <- readOGR(paste(folder, "/", kml_name, sep = ""), kml_name) 
    border <<- poly
    ...
    for (hour in 0:23) { 
        ...
        for (min in m) {
            t <- c(t, .sun(year, month, day, hour, min, 0, lat, long))
        }
    }
    ...
    for (i in 1:length(l)) {
        ...
        if (i/x >= floor(dl[1]) && i/x <= ceiling(dl[2])) {
            ds <- .shade(dem, normalvector(90-t[[i*2-1]], t[[i*2]]), sombra = dem)
            hs <- hillShade(slope, aspect, t[[i*2-1]], t[[i*2]])
            dc <- crop(ds, poly)
            dm <- mask(dc, poly)
            hc <- crop(hs, poly)
            hm <- mask(hc, poly)
